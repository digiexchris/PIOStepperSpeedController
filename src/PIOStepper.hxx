...
namespace PIOStepper
{
	...
	public:
		...
		PIOStepper(PIO pio, uint sm, uint stepPin, float startSpeedHz, Callback aStoppedCallback = nullptr, Callback aCoastingCallback = nullptr, Callback aAcceleratingCallback = nullptr, Callback aDeceleratingCallback = nullptr);

		void Init(float div, int stepsPerRotation, int32_t acceleration, int32_t deceleration);
		void Start(uint32 targetHz);
		void ForceStop();
		bool Step();
